...
async def assign_channel(
    redis: Redis, channel_id: int, webhooks: Iterator[Tuple[int, int, str]]
) -> NoReturn:
...
async def fetch_channel(redis: Redis, channel_id: int) -> List[Tuple[int, str]]:
    ...
    return [
        (int(id), int(user_id), token)
        for id, user_id, token in (webhook.split("-", 2) for webhook in webhooks)
    ]
...
async def assign_latest_user(
    ...
    if webhook_id is None or webhook_token is None or user_id is None:
        ...
        await redis.set(
            key, f"{user_id}-{webhook_id}-{owner_id}-{webhook_token}", expire=15 * 60
        )


async def fetch_latest_user(
    redis: Redis, channel_id: int
